...
/**
 * Kafka adapter for fulfillment-event publication and the CRAFT event-publishing boundary.
 *
 * <p>Schema validation, producer throttling, idempotence, bounded batching, acks=all and an explicit failure path
 * are all visible in repository code or configuration.</p>
 */
@Component
public class KafkaFulfillmentEventPublisher implements FulfillmentEventPort {
    ...
    public CompletableFuture<Void> publish(FulfillmentEvent event) {
        ...
        try {
            CompletableFuture<SendResult<String, byte[]>> send = kafkaTemplate.send(topic, event.orderId(), schemaValidatedPayload);
