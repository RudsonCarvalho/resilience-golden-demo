...
/** Specific liveness check for a critical in-process executor; unlike a static endpoint, this probe can fail. */
...
public class CoreLivenessHealthIndicator implements HealthIndicator {
    ...
    public CoreLivenessHealthIndicator(@Qualifier("outboundExecutor") ThreadPoolTaskExecutor outboundExecutor) {
    ...
    public Health health() {
        ...
        if (pool.isShutdown() || pool.isTerminated()) {
            return Health.down().withDetail("outboundExecutor", "terminated").build();
        }
        return Health.up()
